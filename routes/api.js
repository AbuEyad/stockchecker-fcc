...
module.exports = function (app) {
  ...
  app.route('/api/stock-prices').get(async function (req, res){
      ...
      if (Array.isArray(stock)){
        ...
        console.log(symbol, latestPrice, req.ip)
        const {symbol: symbol2, latestPrice: latestPrice2} = await getStock(stock[1]);
        console.log(symbol2, latestPrice2)
        ...
        if(!symbol){
          stockData.push({
            rel_likes : firststock.like.length - secondstock.likes.length
          });
        }else{
          stockData.push({
            stock: symbol,
        ...
        if(!symbol2){
          stockData.push({
            rel_likes : secondstock.like.length - firststock.likes.length
          });
        }else{
          stockData.push({
            stock: symbol2,
